...
% Load data from ROI1
load('subgenual.mat')                       % format: subj x run x obj trial t-1 x obj trial t
subgenual = squeeze(mean(subgenual,2));     % average across experimental runs
...
for d = 1:3
    distance_effect(:,d) = mean(subgenual(:,dist==d),2);   
end
